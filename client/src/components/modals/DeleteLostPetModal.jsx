...
import { deleteLostPetAction } from "@/actions/lostPetActions"
import styles from "./DeletePetModal.module.css"

export default function DeletelostPetModal({ isOpen, onClose, onSuccess, lostPet }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await deleteLostPetAction(lostPet.id)
      if (result.success) {
        onSuccess()
        onClose()
        router.back()
      } else {
        setError(result.error || "Failed to delete lostPet")
      }
    } catch (err) {
      setError("An error occurred while deleting the lostPet")
    } finally {
      setIsDeleting(false)
    }
  }
      ...
      <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.modalBody}>
          ...
          <p className={styles.confirmText}>
            Are you sure you want to delete <strong>{lostPet.pet.name}'s Listing</strong>?
          </p>
