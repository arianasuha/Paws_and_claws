...
export default function CartItemCard({ item, onUpdate }) {
  ...
  const updateQuantity = async (newQuantity) => {
    ...
    try {
      ...
      const updatedItems = cartItems.map((cartItem) => ({
        product_id: cartItem.product.id, // Access the product ID from the nested product object
  ...
  const itemTotal = (Number.parseFloat(item.product.price) * quantity).toFixed(2)
