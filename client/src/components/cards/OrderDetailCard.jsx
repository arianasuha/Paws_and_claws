...
import { updateOrderAction, deleteOrderAction } from "@/actions/orderActions"

export default function OrderDetailCard({ order, isAdmin = false, adminView = false }) {
  ...
  const statusOptions = ["none", "delivered", "cancelled"]
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      if (response.error) {
        setUpdateError(response.error.order_status[0])
      } else {
          ...
          {order.order_items.map((item) => (
            <div key={item.id} className={styles.orderItem}>
              <div className={styles.itemInfo}>
                ...
                <span className={styles.itemId}>Product ID: {item.product_id}</span>
              </div>
